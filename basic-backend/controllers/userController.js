...
router.post("/register", async (req, res) => {
  ...
  if (user.username === "Gast" || user.username === "gast") {
    return res
      .status(400)
      .json({
        error: "Username darf aus technischen Gründen nicht 'Gast' sein",
      });
  }
  ...
  if (!emailRegex.test(user.email)) {
    return res
      .status(400)
      .json({ error: "es ist kein gültiges E-Mail Format" });
  }
  if (user.acceptTerms !== true) {
    return res
      .status(400)
      .json({ error: "Nutzungsbedingungen müssen akzeptiert sein" });
  }
...
router.get("/", async (req, res) => {
  await getAllUsers((err, users) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Fehler beim Abrufen der Benutzer" });
    }
...
router.get("/:userId", async (req, res) => {
  ...
  await showUserProfile(userId, (err, user) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Fehler beim Abrufen des Benutzers" });
    }
